...
if __name__=='__main__':
    ...
    with tf.Session(config=config) as sess:
        ...
        '''
        # saver.restore(sess,os.path.join(log_dir,'model.ckpt-9'))
        ...
        for epoch in range(epochs):
            ...
            print(epoch, ' : ', epoch_loss / steps)
            save_path = saver.save(sess, os.path.join(log_dir, 'model2.ckpt'), global_step=epoch)
            print('model save to ', save_path)
            # test
        batch_x = np.reshape(mnist.test.images, [-1, img_Pixels_h, img_Pixels_w, img_Pixels_c])
        print('准确率: ', accuracy.eval({x: batch_x, y: mnist.test.labels}))
        ...
        feat=net
        feat=tf.round(feat)
        feat=sess.run(feat,{x:np.reshape(mnist.test.images[0],[1,img_Pixels_h,img_Pixels_w,img_Pixels_c])})
        print(feat.flatten()[:10])
