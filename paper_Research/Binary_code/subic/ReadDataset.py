...
def job_lists(njobs, nt): # 假设 njobs=64,nt=8
  start_id = np.zeros((nt+1,)) # (9,)
  x = njobs%nt # 0
  f = njobs//nt # 8
  for i in range(nt):
    start_id[i+1]=start_id[i] + f + (np.sum(x>0))
    x-=np.sum(x>0)
  return start_id # [0,1,2,3,4,5,6,7,8]*8
...
def load_images(paths, shape, mean=[103.939, 116.779, 123.68], crop=1, mirroring=False, nt=8):
  ...
  '''
  n = len(paths) # batch_size
  img = [None]*n
  images = np.zeros(((n,)+shape)) # n=64,shape=(224,224,3)  images.shape (64,224,224,3)
  mirror = np.zeros((n,), dtype=int) # [batch_size,]
  if(mirroring): # 镜像
    mirror = np.random.randint(2, size=n) # n个0、1
  def preprocess(idx):
    ...
    if(crop==0):
      ...
      for i in ids:
        ...
        short_edge = np.min(img[i].shape[:2]) # 每个batch中最小的图片边(height or width)
        ...
        if(mirror[i]==1):
          images[i] = cv2.flip(images[i], 1)
  threads = []
  start_id = job_lists(n, nt) # n batch_size ,nt number of threads
  for i in range(nt):
...
class Dataset_images():
  def __init__(self, data_file, shape=(224, 224, 3), mean=[103.939, 116.779, 123.68], counter=0):
    self.mean_img = mean
    self.counter = counter
    self.datapaths = open(data_file).readlines() # 读取图片路径  data_file 格式：/home/wu/xxx.jpg 2 (2对应的label)
    self.nimages = len(self.datapaths) # 图片总数
    img_paths = [None]*self.nimages #
    self.labels = np.zeros((self.nimages,)) # label
    for i in range(self.nimages):
      img_paths[i], self.labels[i] = self.datapaths[i].split() # split()按空格分离
    self.img_paths = np.array(img_paths)
    self.imgshape = shape
  def next_batch(self, bsize, nt=4):
    index = np.random.randint(self.nimages, size=int(bsize)) # 随机选择bsize个样本索引
